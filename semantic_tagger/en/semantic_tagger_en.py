...
class SemanticTagger():
    ...
    def __init__(self):
        ...
        # UNABLE TO USE AS INSTALLABLE PACKAGE.
        ...
        self.usas_tags = dict()
        lines = files(semantic_tagger).joinpath('assets/semtags_subcategories.txt').read_text().split('\n')
        for line in lines:
            self.usas_tags[line.rstrip().split('\t')[0]] = line.rstrip().split('\t')[1]
    ...
    def loading_semantic_tagger(self,
        ...
        languages = {'english':
                         {'yes': {'spacy_lang_model': 'en_core_web_sm',
                                  'exclude': ['ner', 'parser', 'entity_linker', 'entity_ruler', 'morphologizer',
                                              'transformer'],
        ...
        if self.mwe_count == 0:
            ...
            print(
                'Please re-start the kernel if you wish to select a different option (at the top, select Kernel - Restart).')
    ...
    def display_tag_text(self, left_right: str):
        ...
        def on_display_button_clicked(_):
            ...
            with display_out:
                ...
                self.df[left_right] = self.tagged_df[self.tagged_df['text_name'] == self.text_name[left_right]].iloc[:,
                ...
                if inc_usas == ('all',) and inc_pos == ('all',) and inc_mwe == ('all',):
                    ...
                    print(
                        'The below table shows the first {} tokens only. Use the above filter to show tokens with specific tags.'.format(
                            self.token_to_display))
            ...
            with stat_out:
                ...
                count_usas = pd.DataFrame.from_dict(
                    Counter(list(itertools.chain(*self.df[left_right].usas_tags_def.to_list()))),
